 ...
 * Shared across UI, Backend, and WebSocket services
 */
...
export interface WebSocketEvents {
  ...
  "vote-revealed": { roomId: string };
  "vote-reset": { roomId: string };

  // Room settings
  "room-settings-updated": { roomId: string; settings: Partial<Room> };
  "active-story-changed": { roomId: string; storyNodeId: string | null };
  "game-state-updated": { roomId: string; isGameOver: boolean };
  ...
  "timer-update": { roomId: string; nodeId: string; state: unknown };
  "canvas-update": { roomId: string; nodes: CanvasNode[] };
}
