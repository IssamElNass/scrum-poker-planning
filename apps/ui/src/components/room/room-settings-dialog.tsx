...
import { Id, RoomWithRelatedData, User } from "@/lib/types";
import {
  Crown,
  Eye,
  Github,
...
export function RoomSettingsDialog({
  ...
  const [selectedUserForKick, setSelectedUserForKick] = useState<User | null>(
    null
  );
  ...
  const isOwner = roomData.room.ownerId === currentUserId;
  ...
  const handleSaveRoomName = async () => {
    ...
    setIsUpdating(true);
    try {
      const result = await apiClient.updateRoomSettings(roomData.room.id, {
        name: roomName.trim(),
      });

      if (!result.success) {
        throw new Error(result.error || "Failed to update room name");
      }
  ...
  const handleKick = async () => {
    ...
    try {
      const result = await apiClient.kickUser(
        roomData.room.id,
        selectedUserForKick.id
      );

      if (!result.success) {
        throw new Error(result.error || "Failed to kick user");
      }
  ...
  const handleTransferOwnership = async () => {
    ...
    try {
      // TODO: Implement transfer ownership server action
      toast({
        title: "Feature not yet implemented",
        description: "Transfer ownership will be available soon",
      });
  ...
  const otherUsers = roomData.users.filter((user) => user.id !== currentUserId);
  const roomOwner = roomData.users.find(
    (user) => user.id === roomData.room.ownerId
  );
